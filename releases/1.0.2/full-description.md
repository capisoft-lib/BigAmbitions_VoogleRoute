...
Voogle Route extends Voogle Maps with a clear on-ground route line, walking navigation, road-aware driving routes, subway fallback, bookmarks, visit history and optional fast travel. The same Voogle Route 1.0.2 package supports Big Ambitions EA 0.11 and 1.0.
...
Version 1.0.2 focuses on route stability and performance. Failed vehicle routes no longer recalculate indefinitely, road searches reuse bounded memory, and one-way destinations in Industry City can fall back to a nearby reachable arrival road.
...
[b]What's new in 1.0.2[/b]

[list]
[*][b]No more failed-route loops[/b] — identical vehicle and indoor failures retry with backoff, then stop after three attempts until the destination, movement mode or player position changes materially
[*][b]Faster bounded routing[/b] — one cancelable multi-start/multi-target search reuses its arrays and binary heap instead of launching many allocation-heavy A* searches
[*][b]Reachable one-way arrivals[/b] — a bounded direction-diverse fallback finds a nearby reachable road when the closest destination lanes form an inaccessible directed pocket
[*][b]Industry City fixes[/b] — restores the audited Road 213 terminal U-turn and prevents dense Road 236 lanes from hiding a reachable nearby arrival
[*][b]Safe asynchronous results[/b] — canceled or superseded vehicle searches can no longer replace the active request
[*][b]Buffered diagnostics[/b] — file writes are buffered, repeated failures and skipped refreshes are throttled, and expensive messages are skipped when logging is disabled
[*][b]Movement-aware walking routes[/b] — foot routes refresh on a slower cadence and only after useful movement; inactive or detached NavMesh agents are rejected before path calculation
[/list]

[b]Included navigation and UI features[/b]
...
[*][b]Correct bookmark ownership[/b] — History visits do not appear as bookmarks; existing user bookmarks, quick rows and cars are preserved
...
[*][b]One package for EA 0.11 and 1.0[/b] — runtime adapters select the correct game APIs automatically; there is no separate legacy Voogle Route build
[*][b]Big Ambitions 1.0 roads[/b] — refreshed road data and bidirectional route coverage for every Hamptons mansion address
[*][b]Hamptons navigation[/b] — dedicated mansion approaches plus entrance-first WAY OUT / GET OUT routing at property gates
[*][b]Stable route lines[/b] — hybrid rendering keeps foot, vehicle, indoor, subway and city-map paths thin and visible on 1.0
...
[*][b]Hidden during computer use[/b] — the panel and open History hide during computer-game sessions and return to their previous visibility afterward
...
[*]Subscribe to and enable Voogle Route 1.0.2
